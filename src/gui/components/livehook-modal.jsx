...
const LiveHookModal = ({ open, formValues, onClose, onFinish, includePageId }) => {
  ...
  const generateCode = async () => {
    ...
    const code = [
      `try {`,
      `  await WB.Node.Instrumentation.stopLiveHooks()`,
      `} catch (e) { }`,
      ``,
    ...
    code.push("});", "", `await WB.Node.Instrumentation.startLiveHooks(${values.pageId});`);
    return code.join("\n");
  }
  ...
  return (
    <BaseModal title="Create Live Hook" open={open} onCancel={onClose} footer={[
      ...
      <LiveHookForm form={form} formValues={formValues} includePageId={includePageId}/>
    </BaseModal>
  );
}
